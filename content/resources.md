---
title: "Resources"
layout: "single"
url: "/resources/"
---

An up-to-date overview of time invested and costs for the SessionClick project. Updated as the project progresses.

## Time Invested

Time tracked in Toggl starting from April 8, 2026.

| Date | Session | Duration |
|------|---------|----------|
| 2026-04-08 | Continue planning with Claude Code | 1h 15min |
| 2026-04-08 | Set up KMP project, tested build for Android and iOS | 1h 00min |
| 2026-04-09 | Setup KB and describe infrastructure | 1h 12min |
| 2026-04-10 | Android App — first steps | 1h 51min |
| 2026-04-11 | Refine app concept, UI mockups | 1h 17min |
| 2026-04-11 | Main Screen Mockups and layout | 1h 04min |
| 2026-04-11 | Co-coding the next Android step | 2h 00min |
| 2026-04-12 | Design with Stitch, Time and Cost overview | 1h 30min |
| 2026-04-13 | Building the main screen — controls, playlist, responsive layout | 1h 45min |
| **Total** | | **~12h 54min** |

_Note: Time before April 8 was not tracked in Toggl._

## One-Time Costs

| Item | Cost |
|------|------|
| sessionclick.de domain setup fee | €1.99 |
| sessionclick.com domain setup fee | €1.99 |
| musicapps.eu domain setup fee | €1.99 |
| **Total one-time** | **€5.97** |

## Monthly Costs

| Item | Cost/month |
|------|-----------|
| sessionclick.de domain hosting | €0.49 |
| sessionclick.com domain hosting | €1.99 |
| musicapps.eu domain hosting | €1.39 |
| Claude Code subscription (incl. 19% German VAT) | €21.42 |
| **Total monthly** | **€25.29** |

_Budget ceiling: €40/month. Current monthly spend: €25.29 — within budget._

## Tools

### Development IDEs

| Tool | Purpose | Cost |
|------|---------|------|
| [Android Studio](https://developer.android.com/studio) | Android development, Kotlin/KMP, CMake/NDK builds | Free |
| [Xcode](https://developer.apple.com/xcode/) | iOS build verification, SwiftUI (pending) | Free (macOS) |

### Code Editors

| Tool | Purpose | Cost |
|------|---------|------|
| [VS Code](https://code.visualstudio.com) | Blog and KB source editing | Free |

### AI Tools

| Tool | Purpose | Cost |
|------|---------|------|
| [Claude Code](https://claude.ai/code) | Architecture, planning, code review, blog writing | €21.42/month (see above) |
| [Gemini Code Assist](https://developers.google.com/gemini/code-assist) | In-IDE code generation inside Android Studio | Free tier |
| [Google Stitch](https://stitch.withgoogle.com) | UI design exploration and mockups | Free |

### Notes & Project Management

| Tool | Purpose | Cost |
|------|---------|------|
| [Obsidian](https://obsidian.md) | Project notes, concept documents, research | Free (local vault) |
| [Toggl Track](https://toggl.com/track/) | Time tracking | Free tier |

### Design & Screenshots

| Tool | Purpose | Cost |
|------|---------|------|
| [SnagIt](https://www.techsmith.com/screen-capture.html) | Screenshots and mockup captures | One-time purchase (pre-project) |

### Hosting & Infrastructure

| Tool | Purpose | Cost |
|------|---------|------|
| [GitHub](https://github.com/musicapps-kek) | Source code, GitHub Pages (blog), GitHub Actions (KB CI/CD) | Free tier |
| [goneo.de](https://www.goneo.de) | Domain registration and email hosting | See domain costs above |

### Static Site Generators

| Tool | Purpose | Cost |
|------|---------|------|
| [Hugo](https://gohugo.io) (PaperMod theme) | Blog at blog.musicapps.eu | Free |
| [Zensical](https://zensical.org) | Knowledge base at kb.musicapps.eu | Free |
