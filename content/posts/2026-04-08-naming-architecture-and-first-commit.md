...
The reasoning: a lot of what this app does is not platform-specific. Song and setlist models, BPM logic, JSON export/import, freemium rules — all of that can live in a `shared/` module and be used by both platforms without duplication. The iOS port becomes "implement the audio engine and build the SwiftUI UI" rather than "rewrite everything".

The audio engine itself stays platform-specific. Timing precision is the whole point of this app, and that means native audio APIs on both sides — Oboe/AAudio on Android, AVAudioEngine or AudioUnit on iOS. I'm treating the audio engine as the highest-risk component and plan to prototype it early.
...
I created the KMP project in Android Studio, pushed it to GitHub under the musicapps-kek account, and watched Gradle download half the internet. I also started XCode and made sure it can build the iOS target without errors, which needed a few tweaks to the Gradle config (done by Claude).
